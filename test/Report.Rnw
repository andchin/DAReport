...
pointmax = dat[which(abs(diff)==max(abs(diff)))]
...
pointmax1 = dat[which(abs(diff1)==max(abs(diff1)))]
...
legend("bottomright",title = "Legend", legend = c("F",expression(F[n]),expression(sup(abs(F-F[n])),"Fnew","NewDiff")), 
