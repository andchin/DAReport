...
value $t$ the probability $P(D_n\le t)\to H(t)$ when $n \to \infty$. More detail
can be found in the textbook \cite{De02}.
...
\Sexpr{format(Cov,digits=4, nsmall=4)} are calculated.
