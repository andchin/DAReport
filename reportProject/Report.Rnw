...
drawDistance = function(sample,mu,sigma, col) {
	...
	text(pointmax+0.3,(empiric(pointmax,sample)+pnorm(pointmax,mu,sigma))/2,paste("d= ",format(max(abs(diff)), digits=4,nsmall=4)))
}
...
\begin{table}[h!]
\centering
<<testresKSFull, echo=FALSE, term=TRUE, results=tex>>=
cat("\\begin{tabular}{|cccccc|} ")
cat("\\hline ")
cat(paste("variable & test statistic & sig. level & critical value & p-value & rejected\\\\ ", "\n", sep=""))
cat("\\hline ")
...
cat(paste("\\end{tabular}", "\n", sep=""))
@
\caption{Test results of the improved KS test on the whole data sample with ten
initial classes}
\label{tab:KS-full}
\end{table}
